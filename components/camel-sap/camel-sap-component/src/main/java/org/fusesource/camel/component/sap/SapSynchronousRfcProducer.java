...
/**
 * Represents an SAP producer performing a synchronous remote function call (SRFC) to an remote function module (RFM) in SAP. 
 * 
...
public class SapSynchronousRfcProducer extends DefaultProducer {
	...
	protected void checkIfTransacted(Exchange exchange) {
		if (getEndpoint().isTransacted()) {
			...
			SAPDestinationTransaction transaction = new SAPDestinationTransaction(getEndpoint().getDestinationName(), getEndpoint().getDestination());
